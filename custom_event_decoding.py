...
class CustomEventDecoding:
    ...
    Modified so that event decoding performs time shifting also when only start_time end_time columns are provided
    """
    ...
    #     # Extend event if it's shorter than extension_time
    ...
    #         # Adjust boundaries if extension goes out of bounds
    ...
    #     # Ensure event is always included in the extracted interval
    #     min_extraction_start = max(new_start_time, end - self.extracted_interval)
    #     max_extraction_start = min(new_end_time - self.extracted_interval, start)

    #     # Ensure valid range
    #     if min_extraction_start > max_extraction_start:
    #         min_extraction_start, max_extraction_start = start, end - self.extracted_interval

    #     # Randomly select start within the valid range
    #     random_start = random.uniform(min_extraction_start, max_extraction_start)
    ...
    def __call__(self, batch):
        ...
        for b_idx in range(batch_len):
            ...
            if batch.get("detected_events", []) and batch["detected_events"][b_idx]:
                ...
                if self.extension_time:# edited by pp
                    start, end = self._time_shifting(start, end, duration) # edited by pp
            else:
                start, end = None, None
